...
window.onload = () => {
   ...
   phoneMask($.querySelector('.question-form').querySelector('input[name=phone]'));
   altMenu();
   modalFunction();
   swiper();
};
...
var swiper = () => {
   ...
   // var newArticlesSlider = new Swiper('.new-articles__list-wrap', {
   //    init: false,
   //    slideClass: "new-articles__item",
   //    wrapperClass: "new-articles__list",
   //    slidesPerView: 'auto'
   // });
   ...
   //    if ($.body.offsetWidth < 510) newArticlesSlider.init();
   //    window.addEventListener('resize', () => {
   //       if ($.body.offsetWidth < 510) newArticlesSlider.init();
   //       else {
   //          newArticlesSlider.detachEvents();
   //       }
   //    });
   // }
};